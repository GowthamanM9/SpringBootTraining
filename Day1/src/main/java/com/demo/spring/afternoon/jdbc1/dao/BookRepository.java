...
public class BookRepository implements MyJdbcRepository {
    ...
    public Optional<Book> save(Book book) {
        if (this.findById(book.getIsbn()).isPresent()) {
            ...
            int count = jdbcTemplate.update("insert into book(isbn, name, author, genre) values(?,?,?,?)",
                    book.getIsbn(), book.getName(), book.getAuthor(), book.getGenre());
            if (count == 1) {
    ...
    public Optional<Book> update(Book book) {
        try {
            if (this.findById(book.getIsbn()).isPresent()) {
                int count = jdbcTemplate.update("update book set name=?,author=?genre?",
                ...
                throw new BookNotExistException("Book Exist");
            }
